...
if (isset($_POST["limit"], $_POST["start"], $_POST["keyword"], $_POST["orderBy"])) {
  ...
  $result = mysqli_query($koneksi, $query);
    while ($row = mysqli_fetch_array($result)) {
        ...
        $isFavorited = false; // Initialize as false by default
        if (isset($_SESSION['id'])) {
            $id = $_SESSION['id'];
            $queryFavorited = "SELECT * FROM favorit WHERE user_id = $id AND post_id = " . $row["id"];
            $resultFavorited = mysqli_query($koneksi, $queryFavorited);
            $isFavorited = mysqli_num_rows($resultFavorited) > 0;
        }

        // Set the appropriate heart icon class based on the favorited status
        $heartIconClass = $isFavorited ? 'red' : ''; // Use 'red' class if favorited, otherwise empty string
            ...
            <div class="col-md-4 col-sm-6 col-xs-12">
                <div class="featured-item">
                    ...
                    <div class="down-content">
                        <a href="../user/aksi_favorit.php?id=' . $row["id"] . '" class="fa fa-heart card-action favorite-icon ' . $heartIconClass . '"></a>
